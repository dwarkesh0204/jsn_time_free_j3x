...
class PoweradminModelK2ItemSearch extends K2ModelItems
{
	...
	private function getQuery () {
		...
		$query = "SELECT i.*, g.title AS groupname, c.name AS category, v.name AS author, w.name as moderator, u.name AS editor FROM #__k2_items as i";

		$query .= " LEFT JOIN #__k2_categories AS c ON c.id = i.catid"." LEFT JOIN #__usergroups AS g ON g.id = i.access"." LEFT JOIN #__users AS u ON u.id = i.checked_out"." LEFT JOIN #__users AS v ON v.id = i.created_by"." LEFT JOIN #__users AS w ON w.id = i.modified_by";
		...
		if ($search) {
			...
			$query.= ' AND LOWER( i.title ) LIKE \'%' . $search . '%\' ';
		}
