...
(function($){
	$.com_k2_itemlist_category = function(Itemid){
		/**
		...
		this.initVariables = function(){
			...
			this.setData( 'view', 'itemlist' );
			this.setData( 'layout', 'category' );
			...
			if ( this.contextMenu != null ){
				...
				if ( $this.contextMenu.isNew() ){
					...
					$this.contextMenu.addItem( JSNLang.translate('JSN_RAWMODE_COMPONENT_EDIT_ARTICLE') ).addEventHandler("click", function(){
						...
						$this.editArticle($this.currentElement.attr('itemid'));
					});
		...
		this.editArticle = function (itemid){
			$this = this;
			var wWidth  = $(window).width()*0.85;
			var wHeight = $(window).height()*0.8;
			var pop = $.JSNUIWindow
			(
				baseUrl+'administrator/index.php?option=com_k2&view=item&tmpl=component&cid='+itemid,
				{
					modal  : true,
					width  : wWidth, 
					height : wHeight,
