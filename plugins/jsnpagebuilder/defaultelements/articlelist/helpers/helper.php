...
class JSNPbArticleListHelper
{
     ...
     * @return bool
     */
    public static function checkK2Enabled()
    {
        return JComponentHelper::isInstalled('com_k2') === 1 && JComponentHelper::isEnabled('com_k2') === '1';
    }

    /**
     * @return bool
     */
    public static function checkEasyblogEnabled()
    {
        return JComponentHelper::isInstalled('com_easyblog') === 1 && JComponentHelper::isEnabled('com_easyblog') === '1';
    }
    ...
    public static function getArticleListSource()
    {
        $articleListSource = array(
            self::PB_ARTICLE_SOURCE_JOOMLA => JText::_('Joomla article'),
        );
        if (self::checkK2Enabled()) {
            self::loadK2Libraries();
            $articleListSource[self::PB_ARTICLE_SOURCE_K2] = JText::_('K2');
        } else {
            $articleListSource[self::PB_ARTICLE_SOURCE_K2] = array('disable' => true, 'text' => JText::_('K2 - Not Installed or Enable yet'));
        }
        if (self::checkEasyblogEnabled()) {
            self::loadEasyblogLibraries();
            $articleListSource[self::PB_ARTICLE_SOURCE_EASY] = JText::_('EasyBlog');
        } else {
            $articleListSource[self::PB_ARTICLE_SOURCE_EASY] = array('disable' => true, 'text' => JText::_('EasyBlog - Not Installed or Enable yet'));
        }
    ...
    {
        return array(
            self::PB_ARTICLE_SORT_BY_ORDERING     => JText::_('Article Manager Order'),
            self::PB_ARTICLE_SORT_BY_FP_ORDERING  => JText::_('Featured Articles Order'),
            self::PB_ARTICLE_SORT_BY_HITS         => JText::_('Hits'),
            self::PB_ARTICLE_SORT_BY_TITLE        => JText::_('Title'),
            self::PB_ARTICLE_SORT_BY_ID           => JText::_('ID'),
            self::PB_ARTICLE_SORT_BY_ALIAS        => JText::_('Alias'),
            self::PB_ARTICLE_SORT_BY_CREATED      => JText::_('Created Date'),
            self::PB_ARTICLE_SORT_BY_MODIFIED     => JText::_('Modified Date'),
            self::PB_ARTICLE_SORT_BY_PUBLISH_UP   => JText::_('Start Publishing Date'),
            self::PB_ARTICLE_SORT_BY_PUBLISH_DOWN => JText::_('Finish Publishing Date'),
        );
    }
    ...
    public static function getDateFilterType()
    {
        return array(
            'off'      => JText::_('Off'),
            'range'    => JText::_('Date Ranger'),
            'relative' => JText::_('Relative Date'),
        );
    }
    ...
    {
        return array(
            'a.created'    => JText::_('Created Date'),
            'a.modified'   => JText::_('Modified Date'),
            'a.publish_up' => JText::_('Start Publishing Date'),
        );
    }
