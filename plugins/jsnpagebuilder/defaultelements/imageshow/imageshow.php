...
class JSNPBShortcodeImageShow extends IG_Pb_Element {
    ...
    public function __construct() {
        if (JComponentHelper::isInstalled('com_imageshow') === 1 && JComponentHelper::isEnabled('com_imageshow') === '1') {
            include_once JPATH_ROOT . '/administrator/components/com_imageshow/classes/jsn_is_factory.php';
        }
        parent::__construct();
    }
    ...
    function element_config() {
        ...
        $this->config['name'] = JText::_('JSN Image Show');
        $this->config['cat'] = JText::_('Extra');
        $this->config['icon'] = "icon-pb-imageshow";
        $this->config['description'] = JText::_("One Joomla gallery extension for all image presentation needs");

        $this->config['exception'] = array(
            'default_content' => JText::_('JSN Image Show'),
        );
    }
        ...
        $this->items = array(
            'content' => array(
                array(
                    ...
                    "std"     => JText::_('JSN Image Show PB_INDEX_TRICK'),
                    "role"    => "title",
                    "tooltip" => JText::_("Set title for current element"),
                ),
                array(
                    'name'    => JText::_('Showlist'),
            ...
            'styling' => array(
                ...
                array(
                    ...
                    'tooltip'         => JText::_('Set the width of a image show (% or px)'),
                ),
                array(
                    'name'     => JText::_('Overall Height'),
                    'id'       => 'height',
                    'type'     => 'text_number',
                    'std'      => '450',
                    'class'    => 'input-mini',
    ...
    function element_shortcode($attributes = null, $content = null) {
        ...
        if (JComponentHelper::isInstalled('com_imageshow') !== 1 || JComponentHelper::isEnabled('com_imageshow') !== '1') {
            $html_element = JText::_('Please install or enable  JSN Image Show Component before use this element');
